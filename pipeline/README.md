...
**Section structure (per the CSIS input spec):** after validation, `digest.py` enforces the
headers. Four are **always shown** — US, Iran, Lebanon, Israel — and display
`Nothing to Report.` when quiet, so the reader can tell "quiet" from "missed." Thirteen
**conditional** country headers — Bahrain, Egypt, Iraq, Jordan, Kuwait, Oman, Pakistan,
Qatar, Saudi Arabia, Syria, Turkey, UAE, Yemen — appear only when they have Iran-war news,
in that fixed order. **General** (always shown) closes the brief with maritime/shipping data
and cross-cutting items. Edit `ESSENTIAL` / `CONDITIONAL` / `_ALIASES` in `digest.py`.
